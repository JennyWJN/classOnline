...
public class ServletAction extends HttpServlet {
	...
	public void getTeacher(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//检查输入参数是否正确先
		showDebug("[getTeacher]收到页面传过来的参数是："+existResultset+","+action+","+type+","+id+","+timeFrom+","+timeTo);
		/*----------------------------------------数据获取完毕，开始和数据库交互*/
		...
		if(existResultset.equals("1")){
			...
			//如果是新查询
			StudentDao fileDao=new StudentDao();
		...
		if(isAjax){
			...
			showDebug("[getTeacher]结果URL："+url);
			response.sendRedirect(url);
		}
	}
	...
	public void getWork(HttpServletRequest request, HttpServletResponse response) throws Exception {
		HttpSession session = request.getSession();
		String dbName=(String)session.getAttribute("unit_db_name");
		String action = request.getParameter("action");
		String id=request.getParameter("id");
		...
		//这里可以修改成统一一个函数读取变量，下面的session里的attr可以用一个变量代替
		//检查输入参数是否正确先
		showDebug("[getWork]收到页面传过来的参数是："+existResultset+","+action+","+type+","+id+","+timeFrom+","+timeTo);
		/*----------------------------------------数据获取完毕，开始和数据库交互*/
		String creator=(String)session.getAttribute("user_name");
		String createTime=(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date());
		...
		if(existResultset.equals("1")){
			...
			//如果是新查询
			StuteaDao fileDao=new StuteaDao();
			jsonObj=fileDao.getRecord(query);
			session.setAttribute(module+"_"+sub+"_get_record_result",jsonObj);
		}
		jsonObj.put("user_id",userId);
	...
	public void getRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(existResultset.equals("1")){
			//要求提取之前查询结果，如果有就取出来，如果没有就重新查询一次，并且保存进session里
			if(session.getAttribute(module+"_"+sub+"_get_record_result")!=null){
				StuworkDao fileDao=new StuworkDao();
				jsonObj=fileDao.getRecord(query);
				session.setAttribute(module+"_"+sub+"_get_record_result",jsonObj);
			}else{
				//如果没有就报错
	...
	public void exportRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String creator=(String)session.getAttribute("user_name");
		String createTime=(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date());
		String exportStuworkPathName="C:\\upload\\subject\\export\\file_export_"+(new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())+".xls";
		JSONObject jsonObj=null;
		if(existResultset.equals("1")){
			...
			if(session.getAttribute(module+"_"+sub+"_get_record_result")!=null){
				jsonObj=(JSONObject)session.getAttribute(module+"_"+sub+"_get_record_result");
				//取出来以后，导出
				exportData(jsonObj,exportStuworkPathName);
			}else{
				//如果没有就报错
				jsonObj=new JSONObject();
				jsonObj.put("result_code",10);
		...
		if(isAjax){
			...
			String resultMsg="操作已经执行，文件已导出到：“"+exportStuworkPathName+"”，请按返回按钮返回列表页面！";
			int resultCode=0;
	...
	public void exportData(JSONObject jsonObj,String filePathName) throws Exception {
		if(jsonObj!=null){
			...
			String[] cols={"index⊙text⊙序号","time_interval⊙text⊙老师名","count⊙text⊙作业名","color⊙text⊙文件名","color_name⊙text⊙提交时间"};	//name⊙type⊙nick
			ew.CreExcel(jsonObj, "作业管理", cols, filePathName);
			jsonObj.put("result_code",0);
	...
	public void FileUpLoad(HttpServletRequest request, HttpServletResponse response)
		...
		request.getRequestDispatcher("stuwork_add.jsp").forward(request, response);
