package xm07.statistic;

/**
 * Created by NightYuan on 2018/6/17.
 */
...
import xm07.dao.Statistic;
import xm07.dao.StatisticDao;
...
public class ServletAction extends HttpServlet {
    ...
    public void processAction(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if(session.getAttribute("user_role")==null){
            ...
            if (action == null){
                ...
                if (action.equals("stu_statistic_record")) {
                    actionOk=true;
                    try {
                        stu_statisticRecord(request, response);
                    } catch (SQLException e) {
                        e.printStackTrace();
                    } catch (JSONException e) {
                        e.printStackTrace();
                ...
                if (action.equals("teastu_statistic_record")) {
                    actionOk=true;
                    try {
                        teastu_statisticRecord(request, response);
                    } catch (SQLException e) {
                        e.printStackTrace();
                    } catch (JSONException e) {
                        e.printStackTrace();
    ...
    public void statisticRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String fileName=getStatisticGraph(session,dbName,jsonObj,"发布作业统计图","测试Column",statisticType,1,1280,768);
        ...
        if(isAjax){
            ...
            resultUrl=resultPath+"/teawork_statistic_result.jsp";
    ...
    public void stu_statisticRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        HttpSession session = request.getSession();
        String action = request.getParameter("action");
        String dbName=(String)session.getAttribute("unit_db_name");
        String type=request.getParameter("type");
        ...
        if(dbName!=null){
            ...
            if(addressId!=null){session.setAttribute(module+"_"+sub+"_statistic_query_addressid",addressId);}
			/*---------------------------------开始进行统计---------------------------*/
            jsonObj=statisticDao.stustatisticRecord(action,dbName,statistic);
            jsonObj.put("user_id",userId);
            jsonObj.put("user_name",userName);
        }
        //获取完毕，开始生成统计图
    ...
    public void teastu_statisticRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
        HttpSession session = request.getSession();
        String action = request.getParameter("action");
        String dbName=(String)session.getAttribute("unit_db_name");
        String type=request.getParameter("type");
        ...
        if(dbName!=null){
            ...
            if(addressId!=null){session.setAttribute(module+"_"+sub+"_statistic_query_addressid",addressId);}
			/*---------------------------------开始进行统计---------------------------*/
            jsonObj=statisticDao.teastu_statisticRecord(action,dbName,statistic);
            jsonObj.put("user_id",userId);
            jsonObj.put("user_name",userName);
        }
        //获取完毕，开始生成统计图
    ...
    public void getFlowStatisticQuerySetup(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        JSONObject jsonObj=new JSONObject();
		/*----------------------------------------数据获取完毕，开始返回数值*/
        if(session.getAttribute("subject_file_statistic_query_time_from")!=null){jsonObj.put("time_from",(String)session.getAttribute("subject_file_statistic_query_time_from"));}else{resultCode=1;}
        if(session.getAttribute("subject_file_statistic_query_time_to")!=null){jsonObj.put("time_to",(String)session.getAttribute("subject_file_statistic_query_time_to"));}else{}
        if(session.getAttribute("subject_file_statistic_query_timeinterval")!=null){jsonObj.put("time_interval",(String)session.getAttribute("subject_file_statistic_query_timeinterval"));}else{}
        if(session.getAttribute("subject_file_statistic_query_addressid")!=null){jsonObj.put("address_id",(String)session.getAttribute("subject_file_statistic_query_addressid"));}else{}
    ...
    public void exportStatisticResultset(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        filePath="C:\\upload\\subject\\export\\temp";
        filePathName=filePath+"\\"+fileName;
        fileUrl="/upload/subject/export/temp/"+fileName;
		/*--------------------赋值完毕--------------------*/
        String dbName=(String)session.getAttribute("unit_db_name");
        String action = request.getParameter("action");
        String existResultset= request.getParameter("exist_resultset");
