...
public class FileDao {
	 ...
	 * 功能：返回结果集
	 */
	public JSONObject getRecord(File query) throws SQLException, IOException, JSONException {
		String tableName="subject_file";
		//开始查询数据库
		String resultMsg="ok";
		int resultCode=0;
		...
		try {
			...
			while (rs.next()) {
				...
				list.add(rs.getString("institute"));
				list.add(rs.getString("credit"));
	...
	public JSONObject modifyRecord(File file) throws JSONException{
		...
		try {
			...
			//构造sql语句，根据传递过来的查询条件参数
			String tableName="subject_file";
			String sql="update "+tableName+" set course='"+file.getCourse()+"',institute='"+file.getInstitute()+"',credit='"+file.getCredit()+"',create_time='"+file.getCreateTime()+"' where id="+file.getId();
	...
	public JSONObject addRecord(File file) throws JSONException, SQLException{
		String resultMsg="ok";
		int resultCode=0;
		List jsonList = new ArrayList();
		ylx_db query_db = new ylx_db(file.getDbName());
		//构造sql语句，根据传递过来的查询条件参数
		String tableName="subject_file";
		String sql="insert into "+tableName+"(parent_id,course,institute,credit,user_id,creator,create_time) values('"+file.getParentId()+"','"+file.getCourse()+"','"+file.getInstitute()+
				"','"+file.getCredit()+"','"+file.getUserId()+"','"+file.getCreator()+"','"+file.getCreateTime()+"')";
		query_db.executeUpdate(sql);
		query_db.close();
		//下面开始构建返回的json
		JSONObject jsonObj=new JSONObject();
	 ...
	 */
	private String createGetRecordSql(File query){
		String sql="";
		String where="";
		if(query.getId()!=null && !query.getId().equals("null")){
			where="where id="+query.getId();
		}
		if(query.getCourse()!=null && !query.getCourse().equals("null") && !query.getCourse().isEmpty()){
			if(!where.isEmpty()){
				where=where+" and course like '%"+query.getCourse()+"%'";
			}else{
				where="where course like '%"+query.getCourse()+"%'";
			}
		}
		if(query.getInstitute()!=null && !query.getInstitute().equals("null") && !query.getInstitute().isEmpty()){
			if(!where.isEmpty()){
				where=where+" and institute like '%"+query.getInstitute()+"%'";
			}else{
				where="where institute like '%"+query.getInstitute()+"%'";
			}
		}
		if(query.getCredit()!=null && !query.getCredit().equals("null") && !query.getCredit().isEmpty()){
			if(!where.isEmpty()){
				where=where+" and credit like '%"+query.getCredit()+"%'";
			}else{
				where="where credit like '%"+query.getCredit()+"%'";
			}
		}
