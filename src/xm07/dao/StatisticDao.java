...
public class StatisticDao{
	...
	public JSONObject doAction(String action,String dbName,String id,String content,String creator,String createTime) throws JSONException{
		...
		try {
			...
			String tableName="xm07_statistic";
	...
	public JSONObject stustatisticRecord(String action,String dbName,Statistic statistic) throws SQLException, IOException, JSONException {
		//开始查询数据库
		String resultMsg="ok";
		int resultCode=0;
		List jsonList = new ArrayList();
		...
		try {
			ylx_db query_db = new ylx_db(dbName);
			//构造sql语句，根据传递过来的查询条件参数
			String sql=getstuStatisticRecordSql(statistic);
			ResultSet rs = query_db.executeQuery(sql);
			while (rs.next()) {
				//查询出来有plate_color,time_interval,count,plate_color_name这几项
				List list = new ArrayList();
	...
	public JSONObject teastu_statisticRecord(String action,String dbName,Statistic statistic) throws SQLException, IOException, JSONException {
		//开始查询数据库
		String resultMsg="ok";
		int resultCode=0;
		List jsonList = new ArrayList();
		...
		try {
			ylx_db query_db = new ylx_db(dbName);
			//构造sql语句，根据传递过来的查询条件参数
			String sql=getteastuStatisticRecordSql(statistic);
			ResultSet rs = query_db.executeQuery(sql);
			while (rs.next()) {
				//查询出来有plate_color,time_interval,count,plate_color_name这几项
				List list = new ArrayList();
	...
	private String getStatisticRecordSql(Statistic statistic){
			...
			sql = "select date_format(create_time,\"" + timeInterval + "\") as time_interval,count(*) as count from xm07_teawork a";
	...
	private String getstuStatisticRecordSql(Statistic statistic){
		...
		sql = "select date_format(create_time,\"" + timeInterval + "\") as time_interval,count(*) as count from xm07_stuwork a";
		sql = sql + " where create_time between \"" + statistic.getTimeFrom() + "\" and \"" + statistic.getTimeTo() + "\"";
		sql = sql + " and user_id='" + statistic.getUserId() + "'";
		sql = sql + " group by time_interval order by time_interval";
		return sql;
	...
	private String getteastuStatisticRecordSql(Statistic statistic){
		...
		sql = "select date_format(create_time,\"" + timeInterval + "\") as time_interval,count(*) as count from xm07_teawork a";
		sql = sql + " where create_time between \"" + statistic.getTimeFrom() + "\" and \"" + statistic.getTimeTo() + "\"";
		sql = sql + " and teacher='" + statistic.getUserId() + "'";
		sql = sql + " group by time_interval order by time_interval";
		return sql;
