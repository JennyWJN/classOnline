...
public class TeastuDao {
    ...
    public JSONObject addRecord(Stuwork file) throws JSONException, SQLException{
        ...
        //构造sql语句，根据传递过来的查询条件参数
        String tableName="xm07_stuwork";
        String sql="insert into "+tableName+"(parent_id,user_id,teacher,work_id,filename,creator,create_time) values('"+file.getParentId()+"','"+file.getUserId()+"','"+file.getTeacher()+
                "','"+file.getWorkid()+"','"+file.getFilename()+"','"+file.getCreator()+"','"+file.getCreateTime()+"')";
        query_db.executeUpdate(sql);
        query_db.close();
        //下面开始构建返回的json
        JSONObject jsonObj=new JSONObject();
    ...
    private String createGetRecordSql(Stuwork query){
        ...
        if(query.getStudent()!=null && !query.getStudent().equals("null") && !query.getStudent().isEmpty()){
            if(!where.isEmpty()){
                where=where+" and user_name like '%"+query.getStudent()+"%'";
            }else{
                where="where user_name like '%"+query.getStudent()+"%'";
            }
        }
        ...
        if(query.getType()!=null && query.getType().equals("all") && query.getUserRole().equals("manager")){
            ...
            if(query.getId()!=null && !query.getId().equals("null")){
                ...
                if(where.isEmpty()){
                    sql="select * from "+query.getTableName()+" where teacher='"+query.getUserId()+"' order by create_time desc";
                }else{
                    sql="select * from "+query.getTableName()+" "+where+" and teacher='"+query.getUserId()+"' order by create_time desc";
                }
            }
        }
        return sql;
