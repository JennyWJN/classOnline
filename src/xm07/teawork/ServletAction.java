...
public class ServletAction extends HttpServlet {
	...
	public void getRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String workcontent=request.getParameter("work_content");
		String workcount=request.getParameter("work_count");
		...
		Teawork query=new Teawork();
		query.setId(id);
		query.setAction(action);
		query.setDbName(dbName);
		query.setType(type);
		query.setWorkid(workid);
		query.setWorkcontent(workcontent);
		query.setWorkcount(workcount);
		query.setLimitTime(limittime);
		...
		if(existResultset.equals("1")){
			...
			//如果是新查询
			TeaworkDao fileDao=new TeaworkDao();
	...
	public void stuList(HttpServletRequest request, HttpServletResponse response) throws Exception {
		HttpSession session = request.getSession();
		String dbName = (String) session.getAttribute("unit_db_name");
		String action = request.getParameter("action");
		String id = request.getParameter("id");
		...
		//这里可以修改成统一一个函数读取变量，下面的session里的attr可以用一个变量代替
		//检查输入参数是否正确先
		showDebug("[stuList]收到页面传过来的参数是：" + existResultset + "," + action + "," + type + "," + id + "," + timeFrom + "," + timeTo+ "," + work_id + ",");
		/*----------------------------------------数据获取完毕，开始和数据库交互*/
		String creator = (String) session.getAttribute("user_name");
		String createTime = (new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date());
		Stuwork query = new Stuwork();
		query.setId(id);
		query.setAction(action);
		query.setDbName(dbName);
		query.setType(type);
		query.setTeacher(teacher);
		query.setWorkid(work_id);
		...
		if (existResultset.equals("1")) {
			...
			//如果是新查询
			TeastuDao fileDao = new TeastuDao();
			jsonObj = fileDao.getRecord(query);
			session.setAttribute(module + "_" + sub + "_get_record_result", jsonObj);
		}
		jsonObj.put("user_id", userId);
		...
		if (isAjax) {
			...
			String resultMsg = "操作已经执行，请按返回按钮返回列表页面！";
			int resultCode = 0;
			redirectUrl = "sturecord_list.jsp?exist_resultset=1";
			resultMsg = java.net.URLEncoder.encode(resultMsg, "UTF-8");
			String url = redirectPath + "/" + redirectUrl;
			showDebug("[stuList]结果URL：" + url);
			response.sendRedirect(url);
		}
	}
	...
	public void exportRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String creator=(String)session.getAttribute("user_name");
		String createTime=(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date());
		String exportTeaworkPathName="C:\\upload\\subject\\export\\file_export_"+(new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())+".xls";
		JSONObject jsonObj=null;
		if(existResultset.equals("1")){
			...
			if(session.getAttribute(module+"_"+sub+"_get_record_result")!=null){
				jsonObj=(JSONObject)session.getAttribute(module+"_"+sub+"_get_record_result");
				//取出来以后，导出
				exportData(jsonObj,exportTeaworkPathName);
			}else{
				//如果没有就报错
				jsonObj=new JSONObject();
				jsonObj.put("result_code",10);
		...
		if(isAjax){
			...
			String resultMsg="操作已经执行，文件已导出到：“"+exportTeaworkPathName+"”，请按返回按钮返回列表页面！";
			int resultCode=0;
	...
	public void exportData(JSONObject jsonObj,String filePathName) throws Exception {
		if(jsonObj!=null){
			...
			String[] cols={"index⊙text⊙序号","time_interval⊙text⊙作业名","count⊙text⊙作业内容","color⊙text⊙应交人数","color_name⊙text⊙限制时间","color_name⊙text⊙创建时间"};	//name⊙type⊙nick
			ew.CreExcel(jsonObj, "课程管理", cols, filePathName);
			jsonObj.put("result_code",0);
