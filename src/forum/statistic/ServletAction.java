...
import forum.dao.Statistic;
import forum.dao.StatisticDao;
...
public class ServletAction extends HttpServlet {
	...
	public void getFlowStatisticQuerySetup(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		JSONObject jsonObj=new JSONObject();
		/*----------------------------------------数据获取完毕，开始返回数值*/
		if(session.getAttribute("forum_file_statistic_query_time_from")!=null){jsonObj.put("time_from",(String)session.getAttribute("forum_file_statistic_query_time_from"));}else{resultCode=1;}
		if(session.getAttribute("forum_file_statistic_query_time_to")!=null){jsonObj.put("time_to",(String)session.getAttribute("forum_file_statistic_query_time_to"));}else{}
		if(session.getAttribute("forum_file_statistic_query_timeinterval")!=null){jsonObj.put("time_interval",(String)session.getAttribute("forum_file_statistic_query_timeinterval"));}else{}
		if(session.getAttribute("forum_file_statistic_query_addressid")!=null){jsonObj.put("address_id",(String)session.getAttribute("forum_file_statistic_query_addressid"));}else{}
	...
	public void exportStatisticResultset(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		filePath="C:\\upload\\forum\\export\\temp";
		filePathName=filePath+"\\"+fileName;
		fileUrl="/upload/forum/export/temp/"+fileName;
		/*--------------------赋值完毕--------------------*/
		String dbName=(String)session.getAttribute("unit_db_name");
		String action = request.getParameter("action");
		String existResultset= request.getParameter("exist_resultset");
